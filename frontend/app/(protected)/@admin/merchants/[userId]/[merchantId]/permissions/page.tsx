...
export default function Permission() {
  ...
  const { data, isLoading } = useSWR(
    `/admin/users/permission/${params.merchantId}`,
  );

  // get user block methods list
  const { data: blockMethods, isLoading: isBlockMethodLoading } = useSWR(
    `/admin/users/blacklisted-methods/${params.merchantId}&search=${methodSearch}`,
  );

  // get user block methods list
  const { data: blockGateway, isLoading: isBlockGatewayLoading } = useSWR(
    `/admin/users/blacklisted-gateways/${params.merchantId}&search=${gatewaySearch}`,
  );
