...
export function WithdrawRequestMenu({ row }: { row: any }) {
  const { t } = useTranslation();
  const router = useRouter();
  const pathname = usePathname();

  // handle deposit request
  const handleAcceptWithdrawRequest = () => {
    toast.promise(acceptWithdrawRequest(row.id), {
      loading: t("Loading..."),
      success: (res) => {
        if (!res.status) throw new Error(res.message);
        mutate("/deposit-requests");
        return res.message;
      },
  ...
  const rejectWithdrawRequest = () => {
    toast.promise(declineWithdrawRequest(row.id), {
      loading: t("Loading..."),
      success: (res) => {
        if (!res.status) throw new Error(res.message);
        mutate("/deposit-requests");
    ...
    <DropdownMenu>
      ...
      >
        ...
        <Case condition={row.status === "pending"}>
          <DropdownMenuItem
            onSelect={handleAcceptWithdrawRequest}
            className="my-0 flex h-full items-center gap-1 px-4 py-2 text-base font-medium leading-[22px] data-[highlighted]:bg-success data-[highlighted]:text-success-foreground"
          >
            <TickCircle />
            {t("Approve request")}
          ...
          <DropdownMenuItem
            onSelect={rejectWithdrawRequest}
