...
import { Add, ArrowDown2, Receive, Repeat, Share, Tree } from "iconsax-react";
import { Menu } from "lucide-react";
import { useSearchParams } from "next/navigation";
import { useTranslation } from "react-i18next";

export default function AgentTransactionFilter({
  filter,
}: {
        ...
        <Command className="p-1">
          <CommandList className="max-h-[450px]">
            <CommandGroup className="p-0">
              ...
              <CommandItem
                onSelect={() =>
                  filter("type", "deposit_request", () => setOpen(false))
                }
                className="mx-0 flex items-center gap-2 rounded-none px-4 py-2 text-base font-medium leading-[22px]"
              >
                <Add size={24} />
              ...
              <CommandItem
                onSelect={() =>
                  filter("type", "withdraw_request", () => setOpen(false))
                }
                className="mx-0 flex items-center gap-2 rounded-none px-4 py-2 text-base font-medium leading-[22px]"
              >
                <Receive size="24" />
